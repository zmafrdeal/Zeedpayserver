...
function publicMoneyPoolUrl(reference) {
  return `${PUBLIC_APP_URL}/pool/${encodeURIComponent(reference)}`;
}
...
app.post("/api/v1/collections/card", requireAuth, async (req, res) => {
  ...
  return proxyJson(
    ...
    getCallbackUrl(req.body || {}),
    {
      referenceId: ledger.reference_id,
      merchantId: req.identity.merchant.id,
      paymentLinkUrl: `${PUBLIC_APP_URL}/pay/${encodeURIComponent(ledger.reference_id)}`,
    },
  );
});
    ...
    try {
      ...
      return res.status(201).json({
        ...
        data: {
          referenceId,
          paymentMethod,
          paymentUrl: `${PUBLIC_APP_URL}/pay/${encodeURIComponent(referenceId)}`,
          publicUrl: `${PUBLIC_APP_URL}/pay/${encodeURIComponent(referenceId)}`,
          status: "active",
        },
      });
    } catch (error) {
  ...
  try {
    ...
    return res.json({
      ...
      links: result.rows.map((row) => ({
        ...
        shareUrl: `${PUBLIC_APP_URL}/pay/${encodeURIComponent(row.reference_id)}`,
  ...
  try {
    ...
    return res.json({
      ...
      link: {
        ...
        publicUrl: `${PUBLIC_APP_URL}/pay/${encodeURIComponent(link.reference_id)}`,
...
app.post("/api/v1/public/payment-links/:reference/contribute", async (req, res) => {
  ...
  try {
    ...
    return proxyJson(
      ...
      normalized,
      `${PUBLIC_APP_URL}/pay/${encodeURIComponent(link.reference_id)}`,
