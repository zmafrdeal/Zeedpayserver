...
async function awardDepositCommission(client, transaction) {
  ...
  return creditCommission(client, {
    ...
    amount: moneyRound(Number(transaction.amount) * DEPOSIT_COMMISSION_RATE),
...
app.get("/api/v1/commission", requireAuth, async (req, res) => {
  try {
    ...
    return res.json({
      ...
      minimumWithdrawal: COMMISSION_WITHDRAWAL_MINIMUM,
    });
  } catch (error) {
...
async function executeCommissionWithdrawal(merchantId, requestedAmount) {
  ...
  try {
    ...
    if (amount < COMMISSION_WITHDRAWAL_MINIMUM) {
      const error = new Error(`Commission withdrawals start at ZMW ${COMMISSION_WITHDRAWAL_MINIMUM.toFixed(2)}`);
