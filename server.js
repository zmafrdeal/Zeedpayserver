...
async function getPlatformSettings() {
  ...
  const result = await db.query(
    `INSERT INTO platform_settings (id, deposit_fee_percent, deposit_fee_fixed,
       ...
       investment_maturity_days, investment_minimum)
...
async function executeInternalTransfer(merchant, body) {
  ...
  try {
    ...
    if (toWallet.wallet_kind === "investment") {
      ...
      await client.query(
        "UPDATE wallets SET balance = $1, investment_maturity_at = $2, updated_at = NOW() WHERE id = $3",
        [afterTo, nextMaturity, toWallet.id],
      );
    } else {
    ...
    const [wallets, transactions, links, feedback, earnings, notifications, features] = await Promise.all([
      db.query(
        `SELECT id, wallet_code, name, currency, balance, wallet_kind, wallet_status,
            is_primary, investment_maturity_at, investment_plan_status,
...
app.patch("/api/v1/admin/settings", requireAdmin, async (req, res) => {
  ...
  for (const [key, column] of updates) {
    const value = key === "investmentEnabled" ? Boolean(Number(req.body[key])) : Number(req.body[key]);
    if (
      (key === "investmentEnabled" && ![0, 1].includes(Number(req.body[key]))) ||
      (key !== "investmentEnabled" && (!Number.isFinite(value) || value < 0 || value > 100000)) ||
    ...
    return res.json({
      ...
      wallets: result.rows.map((wallet) => ({
        ...
        kind: wallet.wallet_kind || "primary",
         ...
         investmentPlan: wallet.wallet_kind === "investment" ? investmentPlan(wallet) : null,
