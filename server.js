...
const DEFAULT_WALLET_NAME = "WAITAPP ONLINE ORDERS";
...
function publicMerchant(merchant, wallet) {
  return {
    ...
    businessName: wallet?.name || merchant.business_name,
...
function transactionInput(body, operation, direction) {
  ...
  return {
    referenceId: /^TXN_[A-Z0-9]{12}$/.test(suppliedReference || "")
      ? suppliedReference
      : makeReferenceId(),
    amount,
    feeAmount,
...
async function executeInternalTransfer(merchant, body) {
  ...
  try {
    ...
    const outReference = stringValue(body.referenceId) || randomIdentifier("TXN", 12);
    const inReference = randomIdentifier("TXN", 12);
    await client.query(
      `INSERT INTO wallet_transactions
...
async function createLedgerForRequest(req, res, body, operation, direction) {
  try {
    const source = body.collectionRequest || body;
    if (!/^TXN_[A-Z0-9]{12}$/.test(stringValue(source.referenceId) || "")) {
...
function makeReferenceId() {
  return randomIdentifier("TXN", 12);
}
...
async function forwardLedgerResponse(res, upstreamResponse, ledgerTransaction) {
  ...
  if (payload && typeof payload === "object" && !Array.isArray(payload)) {
    payload.referenceId = payload.referenceId || ledgerTransaction.referenceId;
  }
  const contentType = upstreamResponse.headers.get("content-type");
  if (payload && typeof payload === "object" && !Array.isArray(payload)) {
...
// Serve the two self-contained frontend screens from the same Render service.
// Keeping the UI and API on one origin also makes the default frontend API URL
// (window.location.origin) work without a separate deployment setting.
...
app.get("/api/v1/wallets", requireAuth, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT id, wallet_code, name, currency, balance, is_primary, created_at
