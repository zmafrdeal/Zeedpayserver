...
const API_KEY = process.env.ZEEDPAY_API_KEY || process.env.LIPILA_API_KEY || "";
const CALLBACK_URL =
  process.env.ZEEDPAY_CALLBACK_URL || process.env.LIPILA_CALLBACK_URL || "";
...
function normalizeSupportPhone(value) {
  ...
  if (!digits) return "";
  if (raw.startsWith("0")) return `+260${digits.slice(1)}`;
  return raw.startsWith("+") ? `+${digits}` : `+${digits}`;
}

function validSupportPhone(value) {
  const normalized = normalizeSupportPhone(value);
...
function publicMerchant(merchant, wallet) {
  return {
    ...
    twoFactorEnabled: Boolean(merchant.two_factor_enabled),
    wallet: wallet
      ? {
          id: wallet.id,
          walletId: wallet.id,
...
async function recordSecurityEvent(merchantId, eventType, req, metadata = {}) {
  if (!db || !merchantId) return;
  await db.query(
    `INSERT INTO security_events
      (id, merchant_id, event_type, ip_address, user_agent, metadata)
     VALUES ($1,$2,$3,$4,$5,$6::jsonb)`,
    [
      crypto.randomUUID(),
      merchantId,
      eventType,
      stringValue(req?.ip) || null,
      stringValue(req?.get?.("user-agent")) || null,
...
async function initializeDatabase() {
  ...
  await db.query(`
    ...
    CREATE INDEX IF NOT EXISTS security_events_merchant_idx
      ON security_events (merchant_id, created_at DESC);
  `);

  await db.query("ALTER TABLE merchants ADD COLUMN IF NOT EXISTS account_status TEXT NOT NULL DEFAULT 'active'");
  await db.query("ALTER TABLE merchants ADD COLUMN IF NOT EXISTS two_factor_enabled BOOLEAN NOT NULL DEFAULT FALSE");
  ...
  await db.query("ALTER TABLE platform_settings ADD COLUMN IF NOT EXISTS support_whatsapp_number TEXT NOT NULL DEFAULT ''");
  await db.query("ALTER TABLE platform_settings ADD COLUMN IF NOT EXISTS support_phone_number TEXT NOT NULL DEFAULT ''");
  await db.query("ALTER TABLE platform_settings ADD COLUMN IF NOT EXISTS support_email TEXT NOT NULL DEFAULT ''");
  await db.query("ALTER TABLE platform_settings ADD COLUMN IF NOT EXISTS fee_rules JSONB NOT NULL DEFAULT '[]'::jsonb");
  await db.query(
  ...
  await db.query(
    "ALTER TABLE wallet_transactions ADD COLUMN IF NOT EXISTS provider TEXT",
  );
  await db.query(
  ...
  return row
    ? {
        ...
        investmentPrincipalProtected: Boolean(row.investment_principal_protected),
         feeRules: Array.isArray(row.fee_rules) ? row.fee_rules : [],
         supportWhatsappNumber: normalizeSupportPhone(row.support_whatsapp_number),
         supportPhoneNumber: normalizeSupportPhone(row.support_phone_number),
         supportEmail: normalizeEmail(row.support_email),
      }
    : fallback;
}

function transactionFee(amount, direction, settings = {}) {
  const rules = Array.isArray(settings.feeRules) ? settings.feeRules : [];
  const matchedRule = rules.find((rule) => {
    const ruleDirection = String(rule.direction || "both").toLowerCase();
    const min = Number(rule.min ?? 0);
    const max = rule.max === null || rule.max === undefined || rule.max === "" ? Infinity : Number(rule.max);
    return (ruleDirection === "both" || ruleDirection === direction) &&
      Number(amount) >= min && Number(amount) <= max;
  });
  if (matchedRule) {
    const value = Number(matchedRule.value ?? matchedRule.fee ?? 0);
    const mode = String(matchedRule.mode || matchedRule.type || "fixed").toLowerCase();
    return moneyRound(Math.max(0, mode === "percent" ? (Number(amount) * value) / 100 : value));
  }
...
async function notifyTransaction(transaction, succeeded) {
  ...
  const direction = incoming
    ? "Payment"
    : /settle/i.test(operation)
      ? "Withdrawal"
      : /disbursement/i.test(operation)
        ? "Disbursement"
        : /transfer/i.test(operation)
          ? "Wallet transfer"
          : "Wallet payment";
  ...
  const title = incoming
    ...
    : `${direction} ${succeeded ? "completed" : "failed"}`;
  const message = succeeded
    ? incoming
      ? `${amount} has been credited to your wallet. Reference: ${transaction.reference_id}.`
      : `${amount} has been sent from your wallet. Reference: ${transaction.reference_id}.`
    : `${direction} of ${amount} could not be completed. Reference: ${transaction.reference_id}.`;
  return createNotification({
    ...
    metadata: {
      ...
      target: "index.html#transactions",
    },
  });
}
...
async function createPendingTransaction(merchant, body, operation, direction) {
  ...
  const provider = transactionProvider(body, operation);
  const id = crypto.randomUUID();
  const result = await db.query(
    `INSERT INTO wallet_transactions
      (id, reference_id, merchant_id, wallet_id, operation, direction, amount, fee_amount,
       net_amount, currency, status, pool_id, provider)
     ...
     ON CONFLICT (reference_id) DO UPDATE SET
       ...
       pool_id = COALESCE(EXCLUDED.pool_id, wallet_transactions.pool_id),
        provider = COALESCE(EXCLUDED.provider, wallet_transactions.provider),
       updated_at = NOW()
     RETURNING *`,
    [
      id,
...
async function executeWalletIdTransfer(merchant, body) {
  ...
  const rawWalletId = stringValue(body.toWalletId || body.walletId).replace(/\s+/g, "");
  const walletId = /^wal[_-]/i.test(rawWalletId) ? rawWalletId.toUpperCase() : rawWalletId;
  ...
  try {
    ...
    const sourceResult = await client.query(
      `SELECT * FROM wallets
        WHERE merchant_id = $1
          AND is_primary = TRUE
          AND COALESCE(wallet_kind, 'primary') = 'primary'
          AND currency = 'ZMW'
        LIMIT 1
        FOR UPDATE`,
      [merchant.id],
    ...
    const destinationResult = await client.query(
      `SELECT w.*, m.full_name, m.business_name
        ...
        WHERE (w.wallet_code = $1 OR w.id = $1)
          AND COALESCE(w.wallet_kind, 'primary') NOT IN ('commission', 'investment')
          AND w.currency = 'ZMW'
        LIMIT 1
        FOR UPDATE`,
      [walletId],
...
async function executeInternalTransfer(merchant, body) {
  ...
  try {
    ...
    result = await client.query(
      `SELECT * FROM wallets
        WHERE merchant_id = $1
          AND COALESCE(wallet_kind, 'primary') <> 'commission'
          AND name = ANY($2::text[])
        FOR UPDATE`,
...
function transactionProvider(body = {}, operation = "") {
  const source = body.collectionRequest || body;
  const explicit = [
    body.provider,
    body.network,
  ...
  const providerText = `${explicit} ${operationText}`;

  if (/bank|swift|account(holder)?/.test(providerText) || /bank_disbursement|settlement/.test(operationText)) {
    return "bank";
  }
  if (/airtel/.test(providerText)) return "airtel";
  if (/\bmtn\b/.test(providerText)) return "mtn";
  if (/zamtel|zamcell/.test(providerText)) return "zamtel";
  if (/card|visa|mastercard|credit|debit/.test(providerText) || /payment_link_collection/.test(operationText)) {
    return "card";
  }
  ...
  const digits = String(normalizePhoneNumber(phone) || "").replace(/\D/g, "");
...
async function validateAndProxy(
  ...
  const walletName = stringValue(body.wallet || body.walletName);
  let ledgerBody = walletName
...
app.use((req, res, next) => {
  if (
    ...
    req.path === "/api/v1/public/site-settings"
  ) {
    return next();
  }
...
app.get("/api/v1/public/site-settings", async (req, res) => {
  try {
    const settings = await getPlatformSettings();
    return res.json({
      success: true,
    ...
    console.error("Public site settings failed:", error.message);
    return jsonError(res, 503, "DATABASE_ERROR", "Could not load public site settings.");
  }
});
...
app.post("/api/v1/auth/signup", async (req, res) => {
  ...
  if (referralBonusAwarded) {
    try {
      await createNotification({
        ...
        metadata: { amount: referralBonusAwarded.earning.amount, referredMerchantId: merchantRowId, target: "index.html#refer" },
      });
    } catch (error) {
  ...
  try {
    await createNotification({
      ...
      type: "welcome",
      title: "Welcome to your Zeedpay wallet",
      message: "Your merchant account and primary wallet are ready to use.",
      dedupeKey: `${merchantRowId}:welcome`,
      metadata: { merchantId, target: "index.html#home" },
    });
  } catch (error) {
...
app.post("/api/v1/auth/login", async (req, res) => {
  ...
  try {
    const previousLogin = await db.query(
      `SELECT ip_address FROM security_events
        WHERE merchant_id = $1 AND event_type = 'login_success'
        ORDER BY created_at DESC LIMIT 1`,
      [row.id],
    );
    const previousIp = previousLogin.rows[0]?.ip_address || null;
    await recordSecurityEvent(row.id, "login_success", req, {
      newIp: Boolean(previousIp && previousIp !== req.ip),
    });
    if (previousIp && previousIp !== req.ip) {
      await createNotification({
        ...
        message: "Your Zeedpay account was accessed from a new device or network location.",
        dedupeKey: `${row.id}:new-login:${Date.now()}`,
        metadata: { ipAddress: req.ip, userAgent: req.get("user-agent") || "", target: "index.html#security" },
      });
    }
  } catch (error) {
    console.error("Security activity could not be recorded:", error.message);
  }
  return res.json({
    success: true,
...
app.put("/api/v1/auth/password", requireAuth, async (req, res) => {
  ...
  try {
    await createNotification({
      ...
      message: "Your Zeedpay wallet password was changed successfully.",
      dedupeKey: `${req.identity.merchant.id}:password:${Date.now()}`,
      metadata: { target: "index.html#security" },
    });
  } catch (error) {
...
app.post("/api/v1/commission/withdraw", requireAuth, requireTransactionPin, async (req, res) => {
  try {
    ...
    await createNotification({
      merchantId: req.identity.merchant.id,
      type: "transaction",
      title: "Commission withdrawal completed",
      message: `ZMW ${withdrawal.amount.toFixed(2)} has been credited to your primary wallet.`,
      dedupeKey: `${req.identity.merchant.id}:${withdrawal.referenceId}:success`,
      metadata: { ...withdrawal, target: "index.html#transactions" },
    });
...
app.post("/api/v1/wallet/pin", requireAuth, async (req, res) => {
  ...
  try {
    await createNotification({
      ...
      message: "Your transaction PIN protects transfers, withdrawals and other wallet money-out actions.",
...
app.get("/api/v1/admin/overview", requireAdmin, async (req, res) => {
  try {
    const [users, balances, transactions, fees, feedback, recent] = await Promise.all([
      ...
      db.query(`SELECT COUNT(*)::int AS total,
        ...
        FROM wallet_transactions`),
      db.query(`SELECT
          COALESCE(SUM(amount) FILTER (WHERE currency = 'ZMW'),0) AS zmw,
    ...
    return res.json({
      ...
      fees: {
        collected: Number(fees.rows[0].zmw),
        zmw: Number(fees.rows[0].zmw),
        usd: Number(fees.rows[0].usd),
      },
  ...
  try {
    ...
    const [wallets, transactions, links, feedback, earnings, notifications, features] = await Promise.all([
      ...
      db.query(
        `SELECT t.id, t.reference_id, t.operation, t.direction, t.amount, t.fee_amount,
            ...
            t.provider, t.provider_response, t.created_at, t.updated_at, w.wallet_code,
    ...
    return res.json({
      ...
      transactions: transactions.rows.map((row) => ({
        ...
        provider: row.provider, providerResponse: row.provider_response, created: row.created_at, updated: row.updated_at,
...
app.patch("/api/v1/admin/settings", requireAdmin, async (req, res) => {
  ...
  for (const [key, column] of updates) {
      const isBoolean = ["investmentEnabled", "investmentPrincipalProtected"].includes(key);
     const isContact = ["supportWhatsappNumber", "supportPhoneNumber", "supportEmail"].includes(key);
      const isFeeRules = key === "feeRules";
     const value = isBoolean
        ? Boolean(Number(req.body[key]))
         : isFeeRules
           ? (() => {
               let parsed;
               try { parsed = typeof req.body[key] === "string" ? JSON.parse(req.body[key] || "[]") : req.body[key]; }
               catch { return null; }
               try {
                 if (!Array.isArray(parsed) || parsed.length > 50) return null;
                 for (const rule of parsed) {
                   const min = Number(rule.min ?? 0);
                   const max = rule.max === null || rule.max === undefined || rule.max === "" ? null : Number(rule.max);
                   const value = Number(rule.value ?? rule.fee ?? 0);
                   const mode = String(rule.mode || rule.type || "fixed").toLowerCase();
                   const ruleDirection = String(rule.direction || "both").toLowerCase();
                   if (!Number.isFinite(min) || min < 0 || (max !== null && (!Number.isFinite(max) || max < min)) ||
                       !Number.isFinite(value) || value < 0 || !["fixed", "percent"].includes(mode) ||
                       !["in", "out", "both"].includes(ruleDirection)) return null;
                 }
                 return parsed.map((rule) => ({
                   min: Number(rule.min ?? 0),
                   max: rule.max === null || rule.max === undefined || rule.max === "" ? null : Number(rule.max),
                   value: Number(rule.value ?? rule.fee ?? 0),
                   mode: String(rule.mode || rule.type || "fixed").toLowerCase(),
                   direction: String(rule.direction || "both").toLowerCase(),
                 }));
               } catch { return null; }
             })()
        : isContact
          ? key === "supportEmail"
            ...
            : normalizeSupportPhone(req.body[key])
          : Number(req.body[key]);
    if (
         (isBoolean && ![0, 1].includes(Number(req.body[key]))) ||
         (isFeeRules && !Array.isArray(value)) ||
        (isContact && key === "supportEmail" && value && !validEmail(value)) ||
        (isContact && key !== "supportEmail" && !validSupportPhone(value)) ||
         (!isBoolean && !isContact && !isFeeRules && (!Number.isFinite(value) || value < 0 || value > 100000)) ||
      (key === "investmentMaturityDays" && (!Number.isInteger(value) || value < 1))
    ) {
       return jsonError(res, 400, "BAD_REQUEST", `${key} must be valid.`);
    }
    values.push(value);
  ...
  try {
    ...
    await writeAudit(req.admin, "settings_updated", "platform_settings", "1",
      Object.fromEntries(updates.map(([key]) => [
        key,
         ["investmentEnabled", "investmentPrincipalProtected"].includes(key)
          ? Boolean(Number(req.body[key]))
           : key === "feeRules"
             ? (typeof req.body[key] === "string" ? JSON.parse(req.body[key] || "[]") : req.body[key])
          : ["supportWhatsappNumber", "supportPhoneNumber", "supportEmail"].includes(key)
            ? (key === "supportEmail"
              ? normalizeEmail(req.body[key])
              : normalizeSupportPhone(req.body[key]))
            : Number(req.body[key]),
...
app.patch("/api/v1/admin/payment-links/:id/status", requireAdmin, async (req, res) => {
  const status = stringValue(req.body?.status);
  if (!["active", "inactive", "deleted"].includes(status)) {
    return jsonError(res, 400, "BAD_REQUEST", "Status must be active, inactive, or deleted.");
  }
  try {
    const result = await db.query(
      `UPDATE payment_links SET status = $1, updated_at = NOW()
        WHERE id = $2 OR reference_id = $2
        RETURNING id, reference_id, status`,
      [status, req.params.id],
    );
    if (!result.rows[0]) return jsonError(res, 404, "PAYMENT_LINK_NOT_FOUND", "Payment link was not found.");
    await writeAudit(req.admin, `payment_link_${status}`, "payment_link", result.rows[0].id, { status });
...
app.delete("/api/v1/admin/payment-links/:id", requireAdmin, async (req, res) => {
  try {
    const result = await db.query(
      `UPDATE payment_links SET status = 'deleted', updated_at = NOW()
        WHERE id = $1 OR reference_id = $1
        RETURNING id, reference_id`,
      [req.params.id],
    );
    if (!result.rows[0]) return jsonError(res, 404, "PAYMENT_LINK_NOT_FOUND", "Payment link was not found.");
    await writeAudit(req.admin, "payment_link_deleted", "payment_link", result.rows[0].id, {});
    return res.json({ success: true, deleted: true, id: result.rows[0].id });
  } catch (error) {
    return jsonError(res, 503, "DATABASE_ERROR", "Could not delete payment link.");
  }
});

app.get("/api/v1/admin/money-pools", requireAdmin, async (req, res) => {
  try {
    const result = await db.query(
      `SELECT p.id, p.reference_id, p.title, p.description, p.target_amount, p.collected_amount,
          p.currency, p.payment_method, p.status, p.expires_at, p.created_at,
          m.business_name, m.email,
          COUNT(c.id)::int AS contributions
         FROM money_pools p
         JOIN merchants m ON m.id = p.merchant_id
         LEFT JOIN money_pool_contributions c ON c.pool_id = p.id
        GROUP BY p.id, m.business_name, m.email
        ORDER BY p.created_at DESC LIMIT 300`,
    );
    return res.json({ success: true, pools: result.rows.map((row) => ({
      id: row.id, reference: row.reference_id, title: row.title, description: row.description,
      target: Number(row.target_amount), collected: Number(row.collected_amount), currency: row.currency,
      paymentMethod: row.payment_method, status: row.status, expiresAt: row.expires_at,
      created: row.created_at, contributions: Number(row.contributions), businessName: row.business_name, email: row.email,
    })) });
  } catch (error) {
...
app.patch("/api/v1/admin/money-pools/:id/status", requireAdmin, async (req, res) => {
  const status = stringValue(req.body?.status);
  if (!["ongoing", "paused", "expired", "deleted"].includes(status)) {
    return jsonError(res, 400, "BAD_REQUEST", "Status must be ongoing, paused, expired, or deleted.");
  }
  try {
    const result = await db.query(
      `UPDATE money_pools SET status = $1, updated_at = NOW()
        WHERE id = $2 OR reference_id = $2
        RETURNING id, reference_id, status`,
      [status, req.params.id],
    );
    if (!result.rows[0]) return jsonError(res, 404, "MONEY_POOL_NOT_FOUND", "Money pool was not found.");
    await writeAudit(req.admin, `money_pool_${status}`, "money_pool", result.rows[0].id, { status });
    return res.json({ success: true, pool: result.rows[0] });
  } catch (error) {
    return jsonError(res, 503, "DATABASE_ERROR", "Could not update money pool.");
  }
});

app.get("/api/v1/admin/data-summary", requireAdmin, async (req, res) => {
  try {
    const tables = ["notifications", "feedback", "audit_logs", "payment_links", "money_pools", "wallet_transactions"];
    const counts = {};
    ...
    return jsonError(res, 503, "DATABASE_ERROR", "Could not load cleanup counts.");
  }
});

app.post("/api/v1/admin/data-delete", requireAdmin, async (req, res) => {
  const allowed = new Set(["notifications", "feedback", "audit_logs", "payment_links", "money_pools", "transactions"]);
  const targets = Array.isArray(req.body?.targets) ? [...new Set(req.body.targets.filter((target) => allowed.has(target)))] : [];
  if (!targets.length) return jsonError(res, 400, "BAD_REQUEST", "Choose at least one cleanup target.");
  if (req.body?.confirmation !== "DELETE") return jsonError(res, 400, "CONFIRMATION_REQUIRED", "Type DELETE to confirm cleanup.");
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    const deleted = {};
    if (targets.includes("notifications")) {
      deleted.notifications = (await client.query("DELETE FROM notifications")).rowCount;
    }
    if (targets.includes("feedback")) {
      deleted.feedback = (await client.query("DELETE FROM feedback")).rowCount;
    }
    if (targets.includes("audit_logs")) {
      deleted.audit_logs = (await client.query("DELETE FROM audit_logs")).rowCount;
    }
    if (targets.includes("payment_links")) {
      deleted.payment_links = (await client.query("UPDATE payment_links SET status = 'deleted', updated_at = NOW() WHERE status <> 'deleted'")).rowCount;
    }
    if (targets.includes("money_pools")) {
      deleted.money_pools = (await client.query("UPDATE money_pools SET status = 'deleted', updated_at = NOW() WHERE status <> 'deleted'")).rowCount;
    }
    if (targets.includes("transactions")) {
      await client.query("DELETE FROM money_pool_contributions");
      await client.query("DELETE FROM platform_fees");
      await client.query("DELETE FROM wallet_ledger");
      deleted.transactions = (await client.query("DELETE FROM wallet_transactions")).rowCount;
    }
    await client.query("COMMIT");
    await writeAudit(req.admin, "database_cleanup", "database", null, { targets, deleted });
    return res.json({ success: true, deleted });
  } catch (error) {
    await client.query("ROLLBACK");
    return jsonError(res, 409, "CLEANUP_FAILED", error.message || "The selected records could not be removed.");
  } finally {
    client.release();
  }
});
...
app.post("/api/v1/admin/investments/:id/refund", requireAdmin, async (req, res) => {
  const client = await db.connect();
  try {
    await client.query("BEGIN");
    const investmentResult = await client.query(
      `SELECT w.*, m.id AS merchant_id
         FROM wallets w JOIN merchants m ON m.id = w.merchant_id
        WHERE w.id = $1 AND w.wallet_kind = 'investment'
        FOR UPDATE`,
      [req.params.id],
    );
    ...
    if (!investment) {
      await client.query("ROLLBACK");
      return jsonError(res, 404, "INVESTMENT_NOT_FOUND", "Investment was not found.");
    }
    const amount = moneyRound(investment.balance);
    const primaryResult = await client.query(
      `SELECT * FROM wallets
        WHERE merchant_id = $1 AND wallet_kind = 'primary' AND currency = $2
        ORDER BY is_primary DESC LIMIT 1 FOR UPDATE`,
      [investment.merchant_id, investment.currency],
    );
    const primary = primaryResult.rows[0];
    if (!primary) throw new Error("The user's primary wallet was not found.");
    const reference = makeReferenceId();
    const transactionId = crypto.randomUUID();
    const before = moneyRound(primary.balance);
    const after = moneyRound(before + amount);
    if (amount > 0) {
      await client.query("UPDATE wallets SET balance = $1, updated_at = NOW() WHERE id = $2", [after, primary.id]);
      await client.query(
        `INSERT INTO wallet_transactions
          (id, reference_id, merchant_id, wallet_id, operation, direction, amount, fee_amount,
           net_amount, currency, status, applied, note)
         VALUES ($1,$2,$3,$4,'investment_refund','in',$5,0,$5,$6,'success',TRUE,$7)`,
        [transactionId, reference, investment.merchant_id, primary.id, amount, investment.currency,
          `Refund for deleted investment ${investment.wallet_code || investment.id}`],
      );
      await client.query(
        `INSERT INTO wallet_ledger
          (id, wallet_transaction_id, merchant_id, wallet_id, entry_type, amount, balance_before, balance_after)
         VALUES ($1,$2,$3,$4,'investment_refund',$5,$6,$7)`,
        [crypto.randomUUID(), transactionId, investment.merchant_id, primary.id, amount, before, after],
      );
    }
    await client.query(
      `UPDATE wallets
          SET balance = 0, investment_principal = 0, investment_plan_status = 'cancelled',
              investment_maturity_at = NULL, updated_at = NOW()
        WHERE id = $1`,
      [investment.id],
    );
    await client.query("COMMIT");
    await writeAudit(req.admin, "investment_refunded_deleted", "wallet", investment.id,
      { refundAmount: amount, refundReference: reference });
    return res.json({ success: true, refunded: amount, reference });
  } catch (error) {
    await client.query("ROLLBACK").catch(() => {});
    return jsonError(res, 409, "INVESTMENT_REFUND_FAILED", error.message || "The investment could not be refunded.");
  } finally {
    client.release();
  }
});

app.delete("/api/v1/admin/investments/:id", requireAdmin, async (req, res) => {
  req.url = `/api/v1/admin/investments/${encodeURIComponent(req.params.id)}/refund`;
  req.method = "POST";
...
app.get(["/terms", "/terms-and-conditions"], (req, res) => {
  return res.redirect("/terms.html");
});

app.get("/terms.html", (req, res) => {
  const file = frontendAsset("terms.html");
  if (!file) return jsonError(res, 404, "FRONTEND_NOT_HOSTED_HERE", "terms.html is hosted by the separate static site");
  return res.sendFile(file);
});
...
app.get("/privacy.html", (req, res) => {
  const file = frontendAsset("privacy.html");
  if (!file) return jsonError(res, 404, "FRONTEND_NOT_HOSTED_HERE", "privacy.html is hosted by the separate static site");
  return res.sendFile(file);
});
...
app.get("/api/v1/wallets/lookup", requireAuth, async (req, res) => {
  const rawWalletId = stringValue(req.query.walletId || req.query.id).replace(/\s+/g, "");
  const walletId = /^wal[_-]/i.test(rawWalletId) ? rawWalletId.toUpperCase() : rawWalletId;
  if (!walletId) return jsonError(res, 400, "BAD_REQUEST", "Enter a wallet ID");
  try {
    const result = await db.query(
      `SELECT w.id, w.wallet_code, w.name, m.full_name, m.business_name
         FROM wallets w
         JOIN merchants m ON m.id = w.merchant_id
        WHERE (w.wallet_code = $1 OR w.id = $1)
          AND COALESCE(w.wallet_kind, 'primary') NOT IN ('commission', 'investment')
  ...
  try {
    ...
    try {
      await createNotification({
        ...
        message: `${transfer.currency} ${transfer.amount.toFixed(2)} has been transferred from your wallet.`,
        dedupeKey: `${req.identity.merchant.id}:${transfer.referenceId}:success`,
        metadata: { ...transfer, target: "index.html#transactions" },
      });
    } catch (error) {
    ...
    if (transfer.recipientMerchantId && transfer.recipientMerchantId !== req.identity.merchant.id) {
      try {
        await createNotification({
          ...
          message: `${transfer.currency} ${transfer.amount.toFixed(2)} has been credited to your wallet.`,
  ...
  try {
    ...
    return res.json({
      ...
      transactions: result.rows.map((row) => ({
        id: row.reference_id,
        reference: row.reference_id,
         displayReference: row.operation === "money_pool_mobile_collection"
           ? "Fundraiser Contribution-Mobile money"
           : row.operation === "money_pool_card_collection" ? "Fundraiser Contribution-card" : row.reference_id,
        ledgerId: row.id,
        operation: row.operation,
        note: row.note || null,
        direction: row.direction,
        ...
        provider: row.provider,
         account: row.donor_phone || null,
         method: row.pool_method === "mobile_money" ? "Mobile Money" : row.pool_method === "card" ? "Card" : row.provider,
        applied: row.applied,
...
app.get("/api/v1/notifications/stream", requireAuth, async (req, res) => {
  res.status(200);
  res.set({
    ...
    try {
      const result = await db.query(
        `SELECT id, type, title, message, priority, is_read, metadata, created_at
          ...
          ORDER BY created_at ASC
          LIMIT 25`,
        [req.identity.merchant.id, since],
      );
      for (const row of result.rows) {
      ...
      if (!result.rows.length) res.write(": ping\n\n");
    } catch (error) {
      if (!closed) res.write(`event: error\ndata: ${JSON.stringify({ message: "Notification stream unavailable" })}\n\n`);
    } finally {
      checking = false;
    }
  };
...
app.get("/api/v1/security/activity", requireAuth, async (req, res) => {
  try {
    const result = await db.query(
    ...
    return jsonError(res, 503, "DATABASE_ERROR", "Could not load security activity");
  }
});

app.patch("/api/v1/security/two-factor", requireAuth, async (req, res) => {
  const enabled = req.body?.enabled;
  if (typeof enabled !== "boolean") {
    return jsonError(res, 400, "BAD_REQUEST", "Two-factor status must be true or false");
  }
  try {
    const result = await db.query(
      `UPDATE merchants SET two_factor_enabled = $1, updated_at = NOW()
        WHERE id = $2
        RETURNING two_factor_enabled`,
      [enabled, req.identity.merchant.id],
    );
    if (!result.rows[0]) return jsonError(res, 404, "MERCHANT_NOT_FOUND", "Merchant was not found");
    await recordSecurityEvent(req.identity.merchant.id, enabled ? "two_factor_enabled" : "two_factor_disabled", req);
    await createNotification({
      merchantId: req.identity.merchant.id,
      type: "security",
      title: enabled ? "Two-factor enabled" : "Two-factor disabled",
      message: enabled
        ? "Two-factor authentication protection is now enabled for your account."
        : "Two-factor authentication protection was disabled for your account.",
      dedupeKey: `${req.identity.merchant.id}:2fa:${enabled}:${Date.now()}`,
    });
    return res.json({ success: true, enabled: result.rows[0].two_factor_enabled });
  } catch (error) {
    console.error("Two-factor setting update failed:", error.message);
    return jsonError(res, 503, "DATABASE_ERROR", "Could not update two-factor settings");
  }
});
...
app.get("/api/v1/fees/preview", requireAuth, async (req, res) => {
  const amount = Number(req.query.amount);
  const direction = String(req.query.direction || "out").toLowerCase() === "in" ? "in" : "out";
  if (!Number.isFinite(amount) || amount <= 0) {
    return jsonError(res, 400, "BAD_REQUEST", "A positive amount is required.");
  }
  try {
    const settings = await getPlatformSettings();
    const fee = moneyRound(transactionFee(amount, direction, settings));
    return res.json({
      success: true,
      amount: moneyRound(amount),
      fee,
      net: moneyRound(direction === "out" ? amount + fee : amount - fee),
      direction,
      feeRulesApplied: Array.isArray(settings.feeRules) && settings.feeRules.some((rule) =>
        amount >= Number(rule.min || 0) &&
        (rule.max === null || rule.max === undefined || amount <= Number(rule.max)) &&
        (rule.direction === "both" || rule.direction === direction)),
    });
  } catch (error) {
    return jsonError(res, 503, "DATABASE_ERROR", "Could not calculate the transaction fee.");
  }
});
...
app.post("/api/v1/money-pools", requireAuth, async (req, res) => {
  ...
  const suggestedAmountValue = req.body?.suggestedAmount;
  const suggestedAmount = suggestedAmountValue === undefined ||
    suggestedAmountValue === null ||
    String(suggestedAmountValue).trim() === ""
    ? 0
    : Number(suggestedAmountValue);
  ...
  if (
    ...
    return jsonError(
      ...
      400,
      "BAD_REQUEST",
      "A title and target amount greater than 0 are required",
    );
  }
  if (!SUPPORTED_CURRENCIES.has(currency)) {
...
function publicMoneyPoolView(pool) {
  ...
  return {
    ...
    suggestedAmount: Number(pool.suggested_amount || 0),
