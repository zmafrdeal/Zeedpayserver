  ...
  await db.query(
    `INSERT INTO payment_links
      (id, merchant_id, reference_id, title, description, amount, currency,
       customer_email, narration, photo_data, pool_id)
      ...
      try {
        await client.query(
          `INSERT INTO money_pool_contributions
            (id, pool_id, transaction_id, amount, currency, status)
           SELECT $1, p.pool_id, $2, $3, $4, 'success'
             FROM payment_links p
            WHERE p.reference_id = $5
  ...
  try {
    ...
    const result = await db.query(
      `UPDATE wallets
        ...
        WHERE id = $3
        RETURNING *`,
      [nextTerm, nextMaturity, wallet.id],
    );
    return res.json({ success: true, plan: investmentPlan(result.rows[0]) });
  } catch (error) {
...
app.post("/api/v1/money-pools", requireAuth, async (req, res) => {
  ...
  if (!title || !Number.isFinite(targetAmount) || targetAmount <= 0) {
    return jsonError(res, 400, "BAD_REQUEST", "A title and target amount greater than 0 are required");
  }
  if (!SUPPORTED_CURRENCIES.has(currency)) {
  ...
  try {
    const result = await db.query(
      `INSERT INTO money_pools
        (id, merchant_id, reference_id, title, description, target_amount, currency, expires_at)
       VALUES ($1,$2,$3,$4,$5,$6,$7,$8)
       RETURNING id, reference_id, title, description, target_amount, currency,
                 collected_amount, status, expires_at, created_at, updated_at`,
      [
        crypto.randomUUID(),
        req.identity.merchant.id,
        makeReferenceId(),
  ...
  try {
    const pools = await db.query(
      `SELECT p.id, p.reference_id, p.title, p.description, p.target_amount,
              p.currency, p.collected_amount, p.status, p.expires_at,
    ...
    const contributions = await db.query(
      `SELECT c.pool_id, c.amount, c.currency, c.status, c.created_at,
  ...
  try {
    const result = await db.query(
      `SELECT id, reference_id, title, description, amount, currency,
              customer_email, narration, checkout_url, photo_data, pool_id,
